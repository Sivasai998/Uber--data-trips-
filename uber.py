...
df=pd.read_csv(r"C:\Users\nvgsiva sai\Downloads\uber-dash-trips.csv",dtype=object)
    ...
    children=[
        html.Div(className="row",
                ...
                html.Div(className="four columns div-user-controls",
                    children=[html.Img(className="logo", src=app.get_asset_url("download.png")),
                        ...
                        # Inorder change to side-by-side for mobile layout
                        html.Div(className="row",
                            children=[html.Div(className="div-for-dropdown",
                                    children=[# Dropdown for locations on map
                                ...
                                html.Div(className="div-for-dropdown",
                                    children=[# Dropdown to select times
                                        dcc.Dropdown(id="bar-selector",
                ...
                # Columns for app graphs and plots
                html.Div(className="eight columns div-for-charts bg-grey",
                    children=[dcc.Graph(id="map-graph"),
                        html.Div(className="text-padding",
                            children=[
                                "Select the bars on the histogram to section data by time."
                            ],
                        ),
...
def get_selection(month, day, selection):
    ...
    yVal = []
    xSelected = []
    colorVal = ["#F4EC15","#ADFF2F","#00FFFF","#9DE81B","#80E41D","#66E01F","#4CDC20","#34D822","#24D249","#25D042",
                "#26CC58","#28C86D","#29C481","#2AC093","#0db15f","#0cc57b","#6f4ae0","#255f56","#81c885","#690ff1",
                "#9105db","#ec1ef6","#a2f425","#6794d5","#60b457","#3567ae","#56ed96","#79dffa","#e55d25"]
...
def update_total_rides_selection(datePicked, selection):
    ...
    if selection is not None or len(selection) is not 0:
        ...
        totalInSelection = 0
        for x in selection:
            totalInSelection += len(totalList[date_picked.month - 4][date_picked.day - 1][totalList[date_picked.month - 4][date_picked.day - 1].index.hour == int(x)])
        firstOutput = "Total rides in selection: {:,d}".format(totalInSelection)

    if ( datePicked is None
...
def update_histogram(datePicked, selection):
    ...
    layout = go.Layout(bargap=0.01,bargroupgap=0,barmode="group",
